...
- Go 后端 + Vue 管理台 + Docker Compose 单容器部署。
...
- Docker Compose 默认只启动一个 `app` 容器，容器内运行 Nginx、Go 后端和 PostgreSQL；数据库数据保存在 `postgres_data` volume。
...
本地测试环境使用根目录的 `.env.development`：Docker 只跑 PostgreSQL，后端和前端本地直接启动。
...
- 测试数据库：localhost:15432

前端通过根目录 `.env.development` 中的 `VITE_API_BASE` 请求本地后端。
